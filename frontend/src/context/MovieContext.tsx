...
const MovieProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateMoviesState = useMemo(
    () => (id: string, data: Movie) => {
  ...
  const updateMovieActors = useMemo(
    () => (movieId: string, actors: Actor[]) => {
      const updatedMovies = movies.map((movie) => {
        if (movie.id === movieId) {
          return {
            ...movie,
  ...
  const removeActorFromMovie = useMemo(
  ...
  const updateMovieGenres = useMemo(
    () => (movieId: string, genres: Genre[]) => {
      const updatedMovies = movies.map((movie) => {
        if (movie.id === movieId) {
          return {
            ...movie,
  ...
  const removeGenreFromMovie = useMemo(
    () => (movieId: string, genreId: string) => {
      const updatedMovies = movies.map((movie) => {
        if (movie.id === movieId) {
          return {
            ...movie,
