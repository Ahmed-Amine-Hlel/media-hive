...
export const ActorContext = createContext<ActorContextProps | undefined>(
  undefined
);
...
const ActorProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateActorsState = useMemo(
    () => (id: string, data: Actor) => {
  ...
  const saveActorToState = useMemo(
    () => (actor: Actor) => {
  ...
  const removeActorFromState = useMemo(
    () => (id: string) => {
